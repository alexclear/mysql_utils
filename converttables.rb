...
begin
  ...
  schemas = {}
  con.query("SELECT table_name, engine, table_schema FROM information_schema.tables WHERE table_schema NOT LIKE 'mysql' AND table_schema NOT LIKE 'information_schema' AND engine LIKE 'MyISAM'").each do |row|
  ...
  schemas.keys.each do |schema|
    ...
    con_schema.close if con_schema
  end
rescue Mysql::Error => e
  puts "Error code: #{e.errno}"
  puts "Error message: #{e.error}"
